...
import { type CSSProperties, type JSX, useLayoutEffect, useState } from 'react'
...
export async function highlight(code: string, lang: BundledLanguage) {
    const cacheKey = `${lang}:auto:${code.length}:${code.slice(0, 50)}:${code.slice(-50)}`
...
export async function highlightWithTheme(code: string, lang: BundledLanguage, theme: 'light' | 'dark') {
    const cacheKey = `${lang}:${theme}:${code.length}:${code.slice(0, 50)}:${code.slice(-50)}`
...
export default function CodeBlock({ code, lang, initial, maxHeight, preHighlighted, theme, className }: Props) {
    ...
    useLayoutEffect(() => {
        ...
        if (code) {
            const highlighter = theme ? highlightWithTheme(code, lang, theme) : highlight(code, lang)
    ...
    return content ? (
        <div
            ...
            style={{ '--pre-max-height': `${maxHeight}px` } as CSSProperties}>
            {content}
        </div>
    ) : (
