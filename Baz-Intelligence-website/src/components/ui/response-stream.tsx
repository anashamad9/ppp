...
export type ResponseStreamProps = {
    textStream: string | AsyncIterable<string>
    mode?: Mode
    // 1-100, where 1 is slowest and 100 is fastest.
    speed?: number
    className?: string
    onComplete?: () => void
    // Element type to render.
    as?: keyof React.JSX.IntrinsicElements
    // Custom fade duration in ms (overrides speed).
    fadeDuration?: number
    // Custom delay between segments in ms (overrides speed).
    segmentDelay?: number
    // Custom characters per frame for typewriter mode (overrides speed).
    characterChunkSize?: number
}
...
function ResponseStream({ textStream, mode = 'typewriter', speed = 20, className = '', onComplete, as = 'div', fadeDuration, segmentDelay, characterChunkSize }: ResponseStreamProps) {
    ...
    // Styles for the fade animation mode.
    const fadeStyle = `
    @keyframes fadeIn {
      from { opacity: 0; }
      to { opacity: 1; }
